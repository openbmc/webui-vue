...
async function main() {
  ...
  try {
    ...
    execSync(`npx tsc --noEmit ${distEndpointsFile}`, {
      cwd: rootDir,
      stdio: 'pipe',
    });
    console.log('   ✅ TypeScript compilation OK');
  } catch (error: any) {
    console.log('   ⚠️  TypeScript errors (may need type dependencies):');
    console.log(error.stdout?.toString().slice(0, 500) || error.message);
  }
