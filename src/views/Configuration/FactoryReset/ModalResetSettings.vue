<template>
  <b-modal id="modal-reset-settings" ref="modal" centered @hidden="resetForm">
    ...
    <ul class="dashed pl-3">
      <li>{{ $t('pageFactoryReset.modal.resetConfirm.message2') }}</li>
      <li>{{ $t('pageFactoryReset.modal.resetConfirm.message3') }}</li>
      <template v-if="!isShowHypervisor">
        <li>{{ $t('pageFactoryReset.modal.resetConfirm.message4') }}</li>
        <li>{{ $t('pageFactoryReset.modal.resetConfirm.message5') }}</li>
        <li>{{ $t('pageFactoryReset.modal.resetConfirm.message6') }}</li>
      </template>
    </ul>
    <div v-if="hostStatus === 'on'">
      <p class="d-flex">
        <span class="text-danger pr-1"><icon-close /></span>
        {{ $t('pageFactoryReset.modal.resetConfirm.message1') }}
      </p>
      <div class="mb-3">
        <b-form-checkbox
        ...
        >
          {{ $t('pageFactoryReset.modal.resetCondition') }}
        </b-form-checkbox>
