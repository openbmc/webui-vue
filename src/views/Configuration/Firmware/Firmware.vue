  ...
  <b-container fluid="xl">
    ...
    <b-row class="mb-4">
      ...
      <b-col md="10" lg="12" xl="8" class="pr-xl-4">
        ...
        <!-- Switch image -->
        <page-section :section-title="$t('pageFirmware.rebootFromBackup')">
          <dl class="mb-5">
            <dt>
              {{ $t('pageFirmware.imageToChangeTo') }}
              <info-tooltip
                :title="$t('pageFirmware.rebootFromBackupHelperText')"
              />
            </dt>
          ...
          <b-btn
            ...
            type="button"
            variant="primary"
            :disabled="!isHostOff || !isRebootFromBackupAvailable || loading"
          >
...
export default {
  ...
  computed: {
    ...
    isRebootFromBackupAvailable() {
      return this.$store.getters['firmware/isRebootFromBackupAvailable'];
    }
  },
  watch: {
  ...
  methods: {
    ...
    setRebootTimeout() {
      // Set 1 minute timeout to disable
      // page interactions while BMC reboot
      // in progress
      this.startLoader();
