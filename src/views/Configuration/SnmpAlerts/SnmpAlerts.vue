  ...
  computed: {
    ...
    tableItems() {
      ...
      return this.allSnmpDetails.map((subscriptions) => {
        const ip = subscriptions.Destination.includes('://')
          ? subscriptions.Destination.split('/')[2].split(':')[0]
          : subscriptions.Destination.split(':')[0];
        const port = subscriptions.Destination.includes('://')
          ? subscriptions.Destination.split('/')[2].split(':')[1]
          : subscriptions.Destination.split(':')[1];
        return {
          IP: ip,
          Port: port,
          id: subscriptions.Id,
