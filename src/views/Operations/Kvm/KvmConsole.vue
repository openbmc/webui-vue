...
import IconCamera from '@carbon/icons-vue/es/camera/20';
...
export default {
  name: 'KvmConsole',
  components: { StatusIcon, IconLaunch, IconArrowDown, IconCamera },
  props: {
    isFullWindow: {
      type: Boolean,
      default: true,
    ...
    captureScreenshot() {
      ...
      if (canvas) {
        canvas.toBlob((blob) => {
          ...
          link.download = `kvm-screenshot-${ts}.png`;
        ...
        console.warn('Canvas element not found in KVM panel');
      }
    },
    closeTerminal() {
      this.rfb.disconnect();
