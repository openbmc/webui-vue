          ...
          <div
            :class="{
              'form-background p-3':
                ...
                (isManualMode || selectedOperatingMode === 'Normal'),
            }"
          >
            <b-form-group
              :key="key"
            ...
            >
              <b-row v-if="key === 'pvm_system_operating_mode'">
                ...
                <div
                  v-if="isManualMode || selectedOperatingMode === 'Normal'"
...
export default {
  ...
  computed: {
    isManualMode() {
      return this.selectedOperatingMode === 'Manual';
    },
  ...
  created() {
    ...
    if (this.currentOperatingMode === 'Manual') {
      this.onChangeSystemOpsMode('Manual');
    }
  },
  updated() {
  ...
  methods: {
    onChangeSystemOpsMode(value) {
      ...
      if (this.selectedOperatingMode === 'Normal') {
        if (this.currentOperatingMode === 'Normal') {
      ...
      } else if (this.selectedOperatingMode === 'Manual') {
        this.$store.commit(
          'serverBootSettings/setAutomaticRetryConfigValue',
          'Disabled'
        );
