...
// sessionURI is stored from the login Location header, which may be a full
// URL (e.g. "https://bmc-host/redfish/v1/SessionService/Sessions/3") or a
// bare path. Session @odata.id values are always bare paths. Normalise to
// pathname so the comparison works regardless of what the BMC returned.
const currentSessionUri = computed(() => {
  const raw = store.state.authentication.sessionURI;
  if (!raw) return null;
  try {
...
const tableItems = computed(() => {
  ...
  return sessions.value.map((session) => {
    ...
    return {
      ...raw,
      // Store the URI under a plain key so b-table's internal reactive
      // wrapping cannot split the dot-containing '@odata.id' key.
      uri: raw['@odata.id'],
      Context: raw.Context || '-',
      ClientOriginIPAddress:
        raw.ClientOriginIPAddress ||
        raw.Oem?.OpenBMC?.ClientOriginIPAddress ||
        '',
      actions: [
        {
          value: 'disconnect',
          title: isCurrentSession(raw)
...
function isCurrentSession(item) {
  // Use the plain 'uri' key copied from '@odata.id' in tableItems,
  // which survives b-table's internal reactive wrapping.
  // Fall back to '@odata.id' when called directly on a raw session object
  // (e.g. from inside the tableItems computed before the item is built).
  const uri = item?.uri ?? item?.['@odata.id'];
