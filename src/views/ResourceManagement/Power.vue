...
const isPowerCapEnabled = computed(() => powerCapValue.value !== null);
const validationRules = computed(() => ({
  powerCapValue: {
    required: requiredIf(() => isPowerCapEnabled.value),
    withinPowerCapRange(value) {
      // When the field is disabled, skip range validation entirely
      if (!isPowerCapEnabled.value) return true;
...
const v$ = useVuelidate(validationRules, { powerCapValue });

const isPowerCapFieldEnabled = computed({
  get: () => isPowerCapEnabled.value,
  set: (enabled) => {
    if (enabled) {
