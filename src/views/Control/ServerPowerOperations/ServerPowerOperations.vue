        ...
        >
          ...
          <template v-else-if="hostStatus === 'on'">
            ...
            <b-form novalidate class="mb-5" @submit.prevent="rebootServer">
              <b-form-group label="Reboot server">
                <b-form-radio
            ...
            <b-form novalidate @submit.prevent="shutdownServer">
              <b-form-group label="Shutdown server">
                <b-form-radio
      ...
      if (this.form.rebootOption === 'orderly') {
        this.$bvModal
          ...
          .then(confirmed => {
            if (confirmed) {
              this.$store
                .dispatch('controls/hostSoftReboot')
                .catch(({ error }) => this.errorToast(error));
            }
          });
        ...
        this.$bvModal
          ...
          .then(confirmed => {
            if (confirmed) {
              this.$store
                .dispatch('controls/hostHardReboot')
                .catch(({ error }) => this.errorToast(error));
            }
          });
      }
      ...
      if (this.form.shutdownOption === 'orderly') {
        this.$bvModal
          ...
          .then(confirmed => {
            if (confirmed) {
              this.$store
                .dispatch('controls/hostSoftPowerOff')
                .catch(({ error }) => this.errorToast(error));
            }
          });
      }
      if (this.form.shutdownOption === 'immediate') {
        this.$bvModal
          ...
          .then(confirmed => {
            if (confirmed) {
              this.$store
                .dispatch('controls/hostHardPowerOff')
                .catch(({ error }) => this.errorToast(error));
            }
          });
      }
