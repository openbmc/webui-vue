...
export default {
  ...
  data() {
    return {
      form: {
        ...
        powerRestorePolicyValue: this.$store.getters[
          'serverBootSettings/powerRestorePolicyValue'
        ],
        automaticRetryConfigValue: this.$store.getters[
  ...
  watch: {
    ...
    biosAttributes: function (value) {
      let manualObj = {};
      if (
        this.powerRestorePolicyValue === 'AlwaysOff' &&
        this.automaticRetryConfigValue === 'Disabled'
      ) {
        manualObj['pvm_system_operating_mode'] = 'Manual';
      }
      manualObj['pvm_system_operating_mode'] = 'Normal';
      this.form.attributes = { ...value, ...manualObj };
    },
