    ...
    <b-row>
      <b-col sm="8" md="6" xl="12">
        <b-form-group :label="$t('pagePowerRestorePolicy.powerPoliciesLabel')">
          <b-form-radio
            v-for="data in powerRestoreData"
            :key="data.state"
...
export default {
  ...
  data() {
    return {
      PowerRestorePolicyValue: null,
    };
  },
  computed: {
    ...mapGetters({
      powerRestoreData: 'powerControl/powerPoliciesData',
    }),
    currentPowerRestorePolicy: {
      get() {
      ...
      set(policy) {
        this.PowerRestorePolicyValue = policy;
      },
    },
  },
  created() {
  ...
  methods: {
    submitForm() {
      if (!this.PowerRestorePolicyValue) return;
      this.startLoader();
      this.$store
        .dispatch(
          'powerControl/setPowerRestorePolicy',
          this.PowerRestorePolicyValue
        )
