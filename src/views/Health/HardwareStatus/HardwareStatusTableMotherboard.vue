<template>
  <page-section :section-title="$t('pageHardwareStatus.motherboard')">
...
export default {
  ...
  computed: {
    motherboard() {
      return this.$store.getters['chassis/motherboard'];
    },
    items() {
      if (this.motherboard) {
  ...
  created() {
    this.$store.dispatch('chassis/getMotherboardInfo').finally(() => {
      // Emit intial data fetch complete to parent component
      this.$root.$emit('hardwareStatus::motherboard::complete');
    });
  }
};
</script>
