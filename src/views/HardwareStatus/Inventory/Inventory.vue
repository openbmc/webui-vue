...
export default {
  ...
  created() {
    ...
    const bmcManagerTablePromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-bmc-manager-complete', resolve);
    });
    const chassisTablePromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-chassis-complete', resolve);
    });
    const dimmSlotTablePromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-dimm-slot-complete', resolve);
    });
    const fansTablePromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-fans-complete', resolve);
    });
    const powerSuppliesTablePromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-power-supplies-complete', resolve);
    });
    const processorsTablePromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-processors-complete', resolve);
    });
    const serviceIndicatorPromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-service-complete', resolve);
    });
    const systemTablePromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-system-complete', resolve);
    });
    const assemblyTablePromise = new Promise((resolve) => {
      eventBus.$once('hardware-status-assembly-complete', resolve);
    });
