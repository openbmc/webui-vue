...
const BVToastMixin = {
  ...
  methods: {
    ...
    successToast(
      message,
      { title: t = i18n.t('global.status.success'), timestamp, refresh } = {}
    ) {
      const body = this._createBody(message);
      const title = this._createTitle(t, 'success');
      if (refresh) body.push(this._createRefreshAction());
      if (timestamp) body.push(this._createTimestamp());
      this._initToast(body, title, 'success');
    },
    errorToast(
      message,
      { title: t = i18n.t('global.status.error'), timestamp, refresh } = {}
    ) {
      const body = this._createBody(message);
    ...
    warningToast(
      message,
      { title: t = i18n.t('global.status.warning'), timestamp, refresh } = {}
    ) {
      const body = this._createBody(message);
