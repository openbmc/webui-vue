...
export interface DisconnectResult {
  ...
  /** URIs that were successfully disconnected */
  successfulUris: string[];
}

/**
 * Active Redfish sessions with $expand fallback via useRedfishCollection.
 *
 * Sessions are mutable, shared state across browsers/tabs. We override the
 * default useRedfishCollection behavior to ensure fresh data on mount and
 * window focus, preventing stale session lists after page refresh or tab switch.
 *
 * Fixes data freshness issue where cached data is served on remount, causing
 * stale session lists after page refresh.
 * See: https://gerrit.openbmc.org/c/openbmc/webui-vue/+/91253 (comment #81)
 */
export function useSessions() {
  return useRedfishCollection<Session>(SESSIONS_COLLECTION_PATH, {
    ...
    expandLevels: 1,
    queryOptions: {
      // Enable refetch on mount to get fresh data after page refresh
      refetchOnMount: true,
      // Enable refetch on window focus for cross-browser/tab sync
      refetchOnWindowFocus: true,
      // Set a small staleTime (5 seconds) to balance freshness with performance
      staleTime: 5000,
    },
  });
}
...
export function useDisconnectSession() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Track successful URIs from other sessions
      otherUris.forEach((uri, index) => {
      ...
      if (deletesCurrent && currentSessionUri) {
        ...
        if (currentDisconnected) {
          successfulUris.push(currentSessionUri);
        }
      }
      ...
      return { successCount, errorCount, currentDisconnected, successfulUris };
    },
    onSuccess: (result) => {
      // On partial failure, only remove URIs that actually succeeded
      queryClient.setQueriesData(
        { queryKey: ['redfish', 'collection', SESSIONS_COLLECTION_PATH] },
        (old: Session[] | undefined) => {
          if (!old) return [];
          return old.filter(
            (session) => !result.successfulUris.includes(session['@odata.id']),
          );
        },
      );
