...
const DumpsStore = {
  ...
  actions: {
    ...
    async getAllDumps({ commit, dispatch }) {
      return await api
        .all([dispatch('getBmcDumps'), dispatch('getSysHostResourceDumps')])
        .then((response) => {
          const bmcDumps = response[0].data?.Members || [];
          const sysHostAndResourceDumps = response[1].data?.Members || [];
          const allDumps = [...bmcDumps, ...sysHostAndResourceDumps];
