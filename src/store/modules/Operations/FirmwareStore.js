...
const FirmwareStore = defineStore('firmware', {
  ...
  actions: {
    ...
    getActiveHostFirmware() {
      return api
        .get('api/redfish/v1/Systems/system/Bios')
        .then(({ data: { Links } }) => {
          const id = Links?.ActiveSoftwareImage['@odata.id'].split('/').pop();
    ...
    async getFirmwareInventory() {
      const inventoryList = await api
        .get('api/redfish/v1/UpdateService/FirmwareInventory')
        .then(({ data: { Members = [] } = {} }) =>
          Members.map((item) => api.get('api' + item['@odata.id']))
        )
        .catch((error) => console.log(error));
      await api
        .all(inventoryList)
    ...
    getUpdateServiceSettings() {
      api
        .get('api/redfish/v1/UpdateService')
        .then(({ data }) => {
          const applyTime =
    ...
    setApplyTimeImmediate() {
      ...
      return api
        .patch('api/redfish/v1/UpdateService', data)
    ...
    async uploadFirmwareTFTP(fileAddress) {
      ...
      return await api
        .post(
          'api/redfish/v1/UpdateService/Actions/UpdateService.SimpleUpdate',
          data
        )
        .catch((error) => {
          console.log(error);
    ...
    async switchBmcFirmwareAndReboot({ getters }) {
      ...
      return await api
        .patch('api/redfish/v1/Managers/bmc', data)
        .catch((error) => {
          console.log(error);
