...
To add a table dropdown filter, import the `<table-filter>` component and `TableFilterMixin`. Add a `filters` prop to the `<table-filters>` component. It should be an array of filter groups–each required to have a `key`, `label`, and `values` prop.

The `label` value should be the translated filter group label. The `key` will usually match the table column key that should be filtered by. The `values` should be an array of filter values that will render as a list of checkbox items in the dropdown.

The component will emit a `@filter-change` event that will provide the filter group and all selected values in the group. The `getFilteredTableData` method from the `TableFilterMixin` can be used to show the filtered table data.
