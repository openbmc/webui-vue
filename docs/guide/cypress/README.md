...
Before running tests, you must set up your BMC credentials:
...
> **Note:** `cypress.env.json` is in `.gitignore` to protect your credentials. Never commit this file!
...
cypress.env.json              # Your credentials (git ignored)
cypress.env.json.example      # Example config template
```
...
| `loginTimeout` | Login wait timeout (ms)  | 15000   |
...
const timeout = Cypress.env("loginTimeout");

// Example: Login with env variables
cy.get("#username").type(Cypress.env("username"));
cy.get("#password").type(Cypress.env("password"));
cy.contains("Overview", { timeout: Cypress.env("loginTimeout") });
```
