...
if ! command -v socket >/dev/null 2>&1; then
    ...
    npm install  @socketsecurity/cli --save-dev
else
    echo "Socket CLI already installed, skipping."
fi

# 2. Add npm/npx wrapper aliases to shell rc files
printf "Add npm/npx Socket wrapper aliases to ~/.bashrc and ~/.zshrc? [y/N] "
read -r REPLY
case "$REPLY" in
    [yY][eE][sS]|[yY])
        ...
        ;;
    *)
        echo "Skipped npm/npx alias setup."
        ;;
esac
...
if git diff --cached --name-only | grep -qE 'package(-lock)?\.json$'; then
  ...
  if ! socket organization list >/dev/null 2>&1; then
    echo "Socket CLI is not authenticated — skipping scan"
    exit 0
  fi
...
echo "Note: npm/npx aliases are machine-wide. To disable: run 'socket wrapper off' and restart your shell."