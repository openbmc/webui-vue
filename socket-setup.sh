...
# 4. Install the pre-push hook into this clone
...
if [ -n "$REPO_ROOT" ]; then
  ...
  cat > "$HOOK_FILE" << 'EOF'
#!/bin/sh
# pre-push — runs Socket scan when dependency files changed
...
if git diff --name-only @{u}.. 2>/dev/null | grep -qE 'package(-lock)?\.json$'; then
  echo "Dependency files changed — running Socket check..."
  socket ci
  if [ $? -ne 0 ]; then
    echo "Socket scan failed — fix before pushing to Gerrit."
    exit 1
  fi
fi
EOF
