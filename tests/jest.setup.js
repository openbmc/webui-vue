...
config.global.plugins = [
  {
    install(app) {
      ...
      // Provide $eventBus for components still using this.$eventBus
      // eslint-disable-next-line global-require, no-param-reassign
      app.config.globalProperties.$eventBus = require('@/eventBus').default;
      app.mixin({
        beforeCreate() {
          const r = this.$root;
          if (r && !r.$on) r.$on = () => {};
          if (r && !r.$emit) r.$emit = () => {};
          if (!this.$eventBus)
            this.$eventBus = app.config.globalProperties.$eventBus;
        },
      });
    },
  },
