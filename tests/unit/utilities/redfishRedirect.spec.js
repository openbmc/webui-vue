...
describe('redirectToRedfishOrHome', () => {
  ...
  it('calls router.push with a valid SPA route', () => {
    const router = {
      push: vi.fn(),
      resolve: vi.fn().mockReturnValue({ matched: [{}], name: 'overview' }),
    };
  ...
  it('falls back to "/" when next resolves to the "login" route (loop guard)', () => {
    const router = {
      push: vi.fn(),
      resolve: vi.fn().mockReturnValue({ matched: [{}], name: 'login' }),
    };

    redirectToRedfishOrHome('/login', router);

    expect(router.push).toHaveBeenCalledWith('/');
    expect(assign).not.toHaveBeenCalled();
  });

  it('falls back to "/" when next resolves to the "change-password" route (loop guard)', () => {
    const router = {
      push: vi.fn(),
      resolve: vi.fn().mockReturnValue({ matched: [{}], name: 'change-password' }),
    };

    redirectToRedfishOrHome('/change-password', router);

    expect(router.push).toHaveBeenCalledWith('/');
  });

  it('falls back to "/" when next resolves to the "page-not-found" route', () => {
    const router = {
      push: vi.fn(),
      resolve: vi.fn().mockReturnValue({ matched: [{}], name: 'page-not-found' }),
    };

    redirectToRedfishOrHome('/page-not-found', router);
