...
// Import the real guard after mocks are in place.
...
describe('router guard – already-authenticated user visiting /login', () => {
  ...
  it('delegates bmcweb-style ?next= (outside hash) via getNextRedirectPath', () => {
    // The guard always calls getNextRedirectPath(to); that helper reads
    // window.location.search when to.query.next is absent.  Verify the guard
    // passes the resolved value straight through to redirectToRedfishOrHome.
    mockGetNextRedirectPath.mockReturnValue('/redfish/v1/Systems');
